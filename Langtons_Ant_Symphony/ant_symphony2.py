...
Each ant follows its own rule from a provided comma-separated list of rules (e.g., 'LRL,LLRR,RL').
Rules are assigned to ants sequentially, cycling back to the start if more ants than rules.
...
Options:
    ...
    --rules STR            Comma-separated list of movement rules for ants (default: 'RL')
                          'R' = turn right, 'L' = turn left
                          e.g., 'RL,RLR,LLRR' will assign different rules to different ants
...
Example:
    python ant_symphony.py --num_ants 5 --rules "LRL,LLRR" --min_freq 100 --max_freq 3000 --duration 30
    This will create 5 ants with rules: LRL, LLRR, LRL, LLRR, LRL
...
class SoundAnt:
    def __init__(self, grid_size, rule):
        ...
        self.rule = rule.strip()  # Remove any whitespace
        self.grid_size = grid_size
        self.rule_length = len(self.rule)

    def move(self, grid):
        current_color = grid[self.pos]
        
        # Get the rule for the current color, cycling through the rule string
        current_rule = self.rule[current_color % self.rule_length]
        ...
        grid[self.pos] = (current_color + 1) % self.rule_length
...
def generate_ant_symphony(args):
    ...
    # Create ants with cycling through rules
    ants = [SoundAnt(grid_size, rules[i % len(rules)]) for i in range(args.num_ants)]
...
def main():
    ...
    parser.add_argument('--rules', type=str, default='RL', help='Comma-separated list of rules for ant movement (e.g., RL,RLR,LLRR)')
    ...
    print(f"Using {args.num_ants} ants with rules:")
    for i in range(args.num_ants):
        print(f"  Ant {i+1}: {rules[i % len(rules)]}")
